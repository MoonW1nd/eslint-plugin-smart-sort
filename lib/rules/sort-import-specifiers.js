...
module.exports = {
  ...
  create(context) {
    ...
    function printSortedSpecifiers(importNode, sourceCode, sortedNames) {
      ...
      const processedTokenData = processSpecifiersTokens(specifierTokens);
      ...
      const newTokens = constructTokens(sortedNames, processedTokenData);
    ...
    return {
      ImportDeclaration(node) {
        ...
        if (importSpecifiers.length > 1) {
          const specifiersMap = getSpecifiersMap(importSpecifiers);
          ...
          if (!isCorrectSorted) {
            context.report({
              ...
              fix(fixer) {
                return fixer
                  .replaceText(node, printSortedSpecifiers(node, sourceCode, sortedNames));
              },
            });
          }
        }
